...
class HierNet(object):
    ...
    def build_arch(self):
        ...
        # Primary Hierules layer, return tensor with shape [batch_size, 1152, 8, 1]
        with tf.variable_scope('PrimaryHier_layer'):
            primaryHier = HierLayer(num_outputs=32, vec_len=8, with_routing=False, layer_type='CONV')
            Hier1 = primaryHier(conv1, kernel_size=9, stride=2)

        # DigitHier layer, return shape [batch_size, 10, 16, 1]
        with tf.variable_scope('DigitHier_layer'):
            digitHier = HierLayer(num_outputs=self.num_label, vec_len=16, with_routing=True, layer_type='FC')
            self.Hier2 = digitHier(Hier1)
        ...
        with tf.variable_scope('Masking'):
            ...
            self.v_length = tf.sqrt(reduce_sum(tf.square(self.Hier2),
                                               axis=2, keepdims=True) + epsilon)
            self.softmax_v = softmax(self.v_length, axis=1)
            # assert self.softmax_v.get_shape() == [cfg.batch_size, self.num_label, 1, 1]

            # b). pick out the index of max softmax val of the 10 Hier
            ...
            if not cfg.mask_with_y:
                ...
                for batch_size in range(cfg.batch_size):
                    v = self.Hier2[batch_size][self.argmax_idx[batch_size], :]
            ...
            # Method 2. masking with true label, default mode
                ...
                self.masked_v = tf.multiply(tf.squeeze(self.Hier2), tf.reshape(self.Y, (-1, self.num_label, 1)))
                self.v_length = tf.sqrt(reduce_sum(tf.square(self.Hier2), axis=2, keepdims=True) + epsilon)
